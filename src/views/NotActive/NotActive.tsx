...
const NotActive = () => {
  ...
  const send = async () => {
    try {
      const { data } = await axios.put(
        `${REACT_APP_SERVER_URL}/users/resendactivation`,
        { email }
      );
      notifySuccess(data);
    } catch (error: any) {
  ...
  return (
    <div className={styles.failure_container}>
      <FcCancel size={250} />
      <h2>
        Tu cuenta no está activada! Sigue el enlace que enviamos a tu correo
        para activarla.
      </h2>
      <h3>Reenviar correo de activación</h3>

      <input
        className={styles.home_product_button}
        name="email"
        type="text"
        value={email}
      ...
      />
      <button onClick={send} className={styles.home_product_button}>
        Reenviar
      </button>
    </div>
  );
